...
<div class="wrap restore-sec">
	<div class="title">
		<h3> <?php _e('Резервное копирование / Восстановление', 'wp-file-manager'); ?></h3>
	</div>
	...
	<div class="schedule-back">
        ...
        <div class="double-col">
            <h4><?php _e('Опции Резервного копирования', 'wp-file-manager'); ?></h4>
            <div class="inner-col-wrap">
                <div class="inner-col-half">
                    <div class="colmn-div3">
                        <input type="checkbox" name="fm_bkp_database" id="fm_bkp_database" value="5" checked="checked"> <?php _e('Резервная копия Базы данных', 'wp-file-manager'); ?>
                    </div>
                    <div class="colmn-div3">
                        <input type="checkbox" name="fm_bkp_files" id="fm_bkp_files" value="files" checked="checked"> <a href="javascript:void(0)" id="fm_open_files_option"><?php _e('Резервное копирование файлов', 'wp-file-manager'); ?></a>
                        <div id="fm_open_files_options" class="fm_open_files_options">
                            <li><input type="checkbox" id="fm_bkp_plugins" name="fm_bkp_plugins" value="1" checked="checked"><?php _e('Плагины', 'wp-file-manager'); ?></li>
                            <li><input type="checkbox" id="fm_bkp_themes" name="fm_bkp_themes" value="2" checked="checked"><?php _e('Темы', 'wp-file-manager'); ?></li>
                            <li><input type="checkbox" id="fm_bkp_other" name="fm_bkp_other" value="4" checked="checked"><?php _e('Другие каталоги найденные внутри wp-content', 'wp-file-manager'); ?></li>
                        </div>
                    </div>
                    <div class="colmn-div3 inner-col-half">
                        <button id="wpfm-backupnow-button" type="button" class="backup_btn"><?php _e('Запуск', 'wp-file-manager'); ?></button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="double-col">
            <div class="well">
                <b><?php _e('Текущая дата и время', 'wp-file-manager'); ?></b>:  <?php echo date('d | F | Y, H:i');?>
            </div>
        </div>
            ...
            <div class="dlt_backup_popup_tbl">
                <div class="dlt_backup_popup_cel">
                    <div class="dlt_backup_popup_inner">
		                ...
		                <div id="dlt_backup">
                            <h3><?php _e('Удаление файлов', 'wp-file-manager'); ?></h3>
                            <div class="dlt_btn_wrap">
                                <p><?php _e('Вы уверены, что хотите удалить эту резервную копию?', 'wp-file-manager'); ?></p>
                                <button class="dlt_cancel backup_btn_common"><?php _e('Закрыть', 'wp-file-manager'); ?></button>
                                <button class="dlt_confirmed backup_btn_common"><?php _e('Продолжить', 'wp-file-manager'); ?></button>
                            </div>
                        </div>
                ...
                <div class="restore_backup_popup_cel">
                    <div class="restore_backup_popup_inner">
		                ...
		                <div id="restore_backup">
                            <h3><?php _e('Восстановление файлов', 'wp-file-manager'); ?></h3>
                            <div class="restore_btn_wrap">
                                <p><?php _e('Вы уверены, что хотите восстановить эту резервную копию?', 'wp-file-manager'); ?></p>
                                <button class="restore_cancel backup_btn_common"><?php _e('Закрыть', 'wp-file-manager'); ?></button>
                                <button class="restore_confirmed backup_btn_common"><?php _e('Продолжить', 'wp-file-manager'); ?></button>
                            </div>
                        </div>
	...
	<div class="log-message">
		<h3><?php _e('Журнал сообщений', 'wp-file-manager'); ?></h3>
        <p>
        <?php if(isset($backups) && !empty($backups)) { ?>
            <?php _e('Резервная копия успешно завершена', 'wp-file-manager'); ?> (<?php echo $backups[0]->backup_date;?>)
             <?php } else { ?>
                <?php _e('В журнале нет сообщений', 'wp-file-manager'); ?>
             <?php } ?>
		</p>
	</div>

	<div class="existing-back">
		<h3><?php _e('Существующие резервные копии', 'wp-file-manager'); ?> <span><?php echo count($backups);?></span> </h3>
	...
	<div class="backup-main">
		<div class="backup-date">
			<input type="checkbox" class="bkpchkCheckAll"> <span> <?php _e('Дата', 'wp-file-manager'); ?> </span>
		</div>
		<div class="download bck_action">
			 <span> <?php _e('Резервное копирование данных (нажмите, чтобы скачать)', 'wp-file-manager'); ?></span>
	...
	<div class="database-sec <?php echo($count++%2 == 0) ? 'even' : 'odd'?>">
        ...
        <div class="action_ele">
			 <button class="exitBackBtn restore_btn bkpRestoreID" id="<?php echo $backup->id; ?>"><?php _e('Восстановить', 'wp-file-manager'); ?></button>
             <button class="exitBackBtn del_btn bkpDeleteID" id="<?php echo $backup->id; ?>"><?php _e('Удалить', 'wp-file-manager'); ?></button>
             <button class="exitBackBtn log_btn bkpViewLog" id="<?php echo $backup->id; ?>"><?php _e('Подробнее', 'wp-file-manager'); ?></button>
		</div>
	</div>
	<?php } ?>
	<?php } else { ?>
           <p class="no_backup"><?php _e('В настоящее время резервных копий не найдено.', 'wp-file-manager'); ?></p>
          <?php } ?>
	<div class="action-sec">
		<strong> <?php _e('Действия с выбранными резервными копиями', 'wp-file-manager'); ?></strong>
        <button class="exitBackBtn bkpDelete del_btn disabled_btn"><?php _e('Удалить', 'wp-file-manager'); ?></button>
        <button class="exitBackBtn bkpCheckAll restore_btn"><?php _e('Выбрать все', 'wp-file-manager'); ?></button>
        <button class="exitBackBtn bkpUnCheckAll log_btn disabled_btn"><?php _e('Отменить', 'wp-file-manager'); ?></button>
	</div>
<p><i><?php _e('Примечание: резервные копии файлов будут находиться в <code>'.$backup_dirname.'</code>', 'wp-file-manager'); ?></i></p>
</div>
...
jQuery(document).ready(function(){
    ...
    jQuery("#wpfm-backupnow-button").click(function(){
        ...
        jQuery('#fmbkp_console').show().html('<p class="backup_wait">Резервное копирование выполняется, подождите...</p>');
...
jQuery(".bkpDelete").click(function () {
    ...
    if(delarr == '') {
    alert('Выберите резервные копии для удаления!');
    } else {
        var r = confirm("Вы уверены, что хотите удалить выбранные резервные копии?")
        if (r == true) {
            jQuery.ajax({
                type: "POST",
                url: ajax_url,
...
jQuery(".dlt_confirmed").click(function () {
    var bkpId = jQuery(this).attr('id')
        ...
        success: function(response) {
            if(response == "Резервная копия удалена успешно!"){
                jQuery(".dlt_backup_popup").hide();
